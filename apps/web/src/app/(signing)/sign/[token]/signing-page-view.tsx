...
export const SigningPageView = ({
  ...
  const threshold = 5; // Arbitrary threshold for determining if fields are on the same "line"
  const sortedFields = fields.sort((a, b) => {
    if (Math.abs(Number(a.positionY) - Number(b.positionY)) <= threshold) {
  ...
  return (
    <RecipientProvider recipient={recipient} targetSigner={selectedSigner ?? null}>
      <div className="mx-auto w-full max-w-screen-xl">
        <h1 className="mt-4 block text-2xl font-semibold md:text-3xl">{document.title}</h1>
        ...
        <div className="mt-8 grid grid-cols-12 gap-y-8 lg:gap-x-8 lg:gap-y-0">
          <Card className="bg-widget col-span-12 rounded-xl border-[1px] before:rounded-xl lg:col-span-7 xl:col-span-8">
          ...
          <div className="col-span-12 self-baseline lg:col-span-5 xl:col-span-4">
            <SigningForm
              document={document}
              recipient={recipient}
              fields={sortedFields}
        ...
        {recipient.role !== RecipientRole.ASSISTANT && (
          <AutoSign recipient={recipient} fields={sortedFields} />
        )}
